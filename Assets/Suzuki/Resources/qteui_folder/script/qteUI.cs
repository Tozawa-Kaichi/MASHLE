...
public class qteUI : MonoBehaviour
{
    ...
    public float _xnumber = 2f;
    public float _ynumber = 2f;
    private float _n;
        ...
        while (input_loop < 5 || input_number < qte_number)
        {
            ...
            for (; 3 > index_y;)
            {
                for (; 6 > index_x;)
                {
                    ...
                    {
                        var setP = transform.position;
                        if(Screen.width == 1920 && Screen.height == 1080)
                        {
                            setP.x = 240+(240 * index_x);
                            setP.y = 216/2+(240 * Random.Range(1, 4));
                        }
                        else
                        {
                            _n = (240 * Screen.width / 1920);
                            print(_n + "=x");
                            setP.x = (Screen.width / _xnumber)+(_n * index_x);
                            _n = (240 * Screen.height / 1080);
                            print(_n + "=y");
                            setP.y = (Screen.height / _ynumber)/2 + ( _n * Random.Range(1, 4));
                        }
                        setP.z = 0;
                        set_qte[input_number] = Instantiate(qte_button, setP, transform.rotation, parent_button);
